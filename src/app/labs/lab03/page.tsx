...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  'matching':              { label: 'Frame Field Mapping',   color: 'bg-teal-100 text-teal-700' },
  'cli-input':             { label: 'CLI Input',             color: 'bg-gray-800 text-green-400' },
}

const FRAME_REF_QUESTIONS = new Set([5, 6, 7])
const MAC_REF_QUESTIONS = new Set([9, 12, 13])
...
function makeInitialState(): QState {
  return {
    selected: null,
    cliInput: '',
    submitted: false,
...
function StarTopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  const hi = highlightNodes ?? []
  const highlight = (name: string) => hi.includes(name)

  const nodeClass = (name: string) =>
    highlight(name)
      ? 'border-yellow-400 bg-yellow-50 shadow-md ring-2 ring-yellow-300 scale-105'
      : 'border-blue-200 bg-blue-50'

  const DeviceNode = ({ name, emoji, sub }: { name: string; emoji: string; sub?: string }) => (
    <div
      className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[80px] ${nodeClass(name)}`}
    >
      <span className="text-xl">{emoji}</span>
      <span className="text-xs font-bold text-gray-800 text-center">{name}</span>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs text-gray-400 ml-auto">Star LAN</span>
      </div>

      <div className="flex flex-col items-center gap-0 select-none">
        {/* PC-A (top) */}
        <DeviceNode name="PC-A" emoji="💻" sub="Sender" />
        ...
        {/* Switch (center) */}
        <DeviceNode name="Switch" emoji="🔀" sub="Fa0/1–3" />
        ...
        <div className="relative flex items-center w-40 -mt-8 mb-1">
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-px bg-gray-300" />
        </div>

        {/* PC-B and PC-C (bottom) */}
        <div className="flex items-start gap-6 mt-1">
          <DeviceNode name="PC-B" emoji="🖥️" sub="Receiver" />
          <DeviceNode name="PC-C" emoji="🖥️" sub="Receiver" />
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 grid grid-cols-3 gap-1.5">
        {[
          { label: 'Unicast', color: 'bg-blue-100 text-blue-700', desc: '1 → 1' },
          { label: 'Broadcast', color: 'bg-orange-100 text-orange-700', desc: '1 → All' },
          { label: 'Multicast', color: 'bg-purple-100 text-purple-700', desc: '1 → Group' },
        ].map((t) => (
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-800 text-sm">Ethernet Frame Structure</span>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          {/* Visual frame bar */}
          <div className="flex rounded-lg overflow-hidden border border-gray-200 text-center text-xs font-semibold">
            {ETHERNET_FRAME_FIELDS.map((f) => (
              <div
                key={f.name}
                className={`flex-1 py-2 px-1 border-r last:border-r-0 border-gray-200 ${f.color}`}
                title={f.description}
              >
                {f.name}
              </div>
          ...
          <div className="space-y-1.5">
            {ETHERNET_FRAME_FIELDS.map((f) => (
              <div key={f.name} className="flex items-start gap-2">
                <span className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-md border ${f.color}`}>
                  {f.name}
                </span>
                <div className="text-xs text-gray-600 mt-0.5 flex-1">
                  <span className="font-medium text-gray-700">{f.size}</span>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      ...
      {open && (
        <div className="px-4 pb-4 space-y-2">
          {TOPOLOGY_DEVICES.map((d) => (
            <div key={d.name} className="rounded-xl border border-blue-200 bg-blue-50 p-3">
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-primary-50 to-blue-50 rounded-2xl border border-primary-100 shadow-card overflow-hidden">
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-primary-900 leading-relaxed">{LAB03_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-primary-700 uppercase tracking-wider mb-2">
              You must prove you understand:
            </p>
            <ul className="space-y-1.5">
              {LAB03_SCENARIO.goals.map((g) => (
          ...
          <div className="bg-white/70 rounded-xl p-3 border border-primary-100">
            <p className="text-sm text-primary-900 font-medium">{LAB03_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-primary-600">Lab 03</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1">
              <Star className="w-3 h-3 text-yellow-400" />
              {LAB03_META.difficulty}
            </span>
          </div>
...
function CLIInput({
  ...
  useEffect(() => {
    if (!state.submitted && !state.revealed) inputRef.current?.focus()
  }, [state.submitted, state.revealed])
...
function HintPanel({ hints, hintLevel, onRevealHint }: { hints: string[]; hintLevel: number; onRevealHint: () => void }) {
  ...
  return (
    <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 space-y-2">
      ...
      {hintLevel < hints.length && (
        <button onClick={onRevealHint} className="mt-1 text-xs font-semibold text-amber-600 hover:text-amber-700 underline underline-offset-2">
          Show next hint →
        </button>
      )}
    </div>
...
function FeedbackPanel({ isCorrect, explanation, revealed }: { isCorrect: boolean; explanation: string; revealed: boolean }) {
  if (isCorrect) {
    return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-primary-500 to-primary-600 p-6 text-center">
          ...
          <p className="text-primary-100 text-sm mt-1">{LAB03_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB03_COMPLETION.conceptMastered}</p>
            <p className="text-xs text-green-700 mt-1 leading-relaxed">{LAB03_COMPLETION.summary}</p>
            <ul className="mt-2 space-y-1">
              {LAB03_COMPLETION.masteredPoints.map((p) => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">
                {LAB03_COMPLETION.reviewIfNeeded.map((l) => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB03_COMPLETION.nextLab.title}</p>
...
export default function Lab03Page() {
  ...
  const [qState, setQState] = useState<QState>(makeInitialState())
  ...
  const checkAnswer = useCallback(() => {
    ...
    setQState((s) => ({ ...s, submitted: true, isCorrect: correct, attempts: s.attempts + 1 }))
  ...
  const handleReveal = useCallback(() => {
    setQState((s) => ({ ...s, submitted: true, revealed: true, isCorrect: false, cliInput: '' }))
  }, [])

  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab03', score, total)
  ...
  const rawHighlights = TOPO_HIGHLIGHTS[currentQ.id]
  const topoHighlights: string[] =
    rawHighlights && (qState.isCorrect || qState.revealed) ? rawHighlights : []

  const isFrameQuestion = FRAME_REF_QUESTIONS.has(currentQ.id)
  const isMACQuestion = MAC_REF_QUESTIONS.has(currentQ.id)
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <ScenarioCard />
            <StarTopologyCard highlightNodes={topoHighlights.length ? topoHighlights : undefined} />
            <DevicePanel />
            <FrameReferencePanel />
          </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {currentQ.type === 'topology-reasoning' && (
                    <span className="text-xs text-indigo-600 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> Refer to topology
                    </span>
                  )}
                  {currentQ.type === 'cli-input' && (
                    <span className="text-xs text-green-600 flex items-center gap-1">
                      <Terminal className="w-3 h-3" /> Type your command
                    </span>
                  )}
                  {isFrameQuestion && (
                    <span className="text-xs text-teal-600 flex items-center gap-1">
                      <Layers className="w-3 h-3" /> See frame reference
                    </span>
                  )}
                  {isMACQuestion && (
                    <span className="text-xs text-indigo-600 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> MAC addressing
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-1 flex-wrap">
                {QUESTIONS.map((q, i) => (
                  <div
                    ...
                    className={`flex-1 min-w-[14px] h-2.5 rounded-full transition-all duration-300 ${
                      correctSet.has(q.id)  ? 'bg-green-400'
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB03_META.lessonsReinforced.map((l) => (
