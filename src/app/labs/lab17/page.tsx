...
import {
  ...
  LAB17_META, LAB17_SCENARIO, LAB17_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, addr }: { name: string; icon: string; addr?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[110px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-slate-300 bg-slate-50'}`}>
      <span className="text-lg">{icon}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      {addr && <span className="text-xs font-mono text-slate-700">{addr}</span>}
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Port Security &amp; Wireless</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <div className="flex gap-4 justify-center">
          <Node name="Authorized PC" icon="✅" addr="MAC: AA:BB:CC" />
          <Node name="Wireless Clients" icon="📱" addr="802.11" />
        </div>
        ...
        <div className="flex gap-4 justify-center">
          <Node name="Switch" icon="🔀" addr="Port-Security" />
          <Node name="Access Point" icon="📡" addr="CAPWAP" />
        </div>
        <div className="flex gap-6 w-full justify-center">
          <div className="w-px h-3 bg-slate-300" />
          <div className="w-px h-3 bg-teal-300" />
        ...
        <div className="flex gap-4 justify-center">
          <Node name="Router" icon="🌐" addr="192.168.1.1" />
          <Node name="WLC" icon="🎛️" addr="Controller" />
        </div>
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100 space-y-1">
        <div className="flex justify-between text-xs">
          <span className="text-slate-600 font-semibold">Violation Mode</span>
          <span className="text-gray-400">shutdown → err-disabled</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-teal-600 font-semibold">AP ↔ WLC</span>
          <span className="text-gray-400">CAPWAP tunnel</span>
        </div>
      </div>
    </div>
  )
...
// port security ref
function PortSecurityReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">Port Security &amp; Wireless Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Violation Modes</p>
            {[
              { mode: 'shutdown',  color: 'bg-red-50 text-red-700',   desc: 'Port goes err-disabled (default)' },
              { mode: 'restrict',  color: 'bg-amber-50 text-amber-700', desc: 'Drops violation traffic, logs' },
              { mode: 'protect',   color: 'bg-slate-50 text-slate-700', desc: 'Drops silently, no log' },
            ].map(r => (
              <div key={r.mode} className={`rounded-lg px-3 py-1.5 mb-1 ${r.color} border border-current border-opacity-20`}>
                <span className="text-xs font-bold">{r.mode}</span>
                <span className="text-xs opacity-80 ml-2">— {r.desc}</span>
              </div>
            ))}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Key CLI Commands</p>
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-1 text-xs font-mono text-slate-800">
              <p>switchport port-security</p>
              <p>switchport port-security maximum 1</p>
              <p>switchport port-security mac-address sticky</p>
              <p>switchport port-security violation shutdown</p>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Wireless Architecture</p>
            <div className="bg-teal-50 border border-teal-200 rounded-lg p-2 space-y-1 text-xs text-teal-800">
              <p><span className="font-bold">Autonomous AP</span> — self-managed, no WLC needed</p>
              <p><span className="font-bold">Lightweight AP</span> — requires WLC (CAPWAP tunnel)</p>
              <p><span className="font-bold">CAPWAP</span> — UDP 5246 (ctrl) / 5247 (data)</p>
              <p><span className="font-bold">WPA2</span> — AES/CCMP encryption (current standard)</p>
              <p><span className="font-bold">802.1X</span> — port-based auth via EAP + RADIUS</p>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-teal-50 rounded-2xl border border-slate-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-slate-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-slate-900">{LAB17_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB17_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-slate-800 flex items-start gap-2"><span className="text-teal-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-slate-100">
            <p className="text-sm text-slate-900 font-medium">{LAB17_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-600">Lab 17</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB17_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB17_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB17_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-slate-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB17_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-slate-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-slate-500 to-teal-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-slate-300 hover:bg-slate-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-slate-400 bg-slate-50 ring-2 ring-slate-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-slate-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-slate-100 text-slate-700 border-slate-200', 'bg-teal-100 text-teal-700 border-teal-200', 'bg-cyan-100 text-cyan-700 border-cyan-200', 'bg-sky-100 text-sky-700 border-sky-200']
  return (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-slate-500 to-teal-600 p-6 text-center">
          <div className="text-5xl mb-2">🏆</div>
          <h2 className="text-2xl font-bold text-white">Lab Complete!</h2>
          <p className="text-slate-100 text-sm mt-1">{LAB17_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB17_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB17_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB17_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB17_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-600 hover:bg-slate-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab17Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab17', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-slate-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-slate-600 hover:bg-slate-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-slate-600 hover:bg-slate-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-slate-600">{score} correct</span>
              </div>
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-slate-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB17_META.lessonsReinforced.map(l => (
