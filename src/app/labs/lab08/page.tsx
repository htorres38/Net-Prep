...
import {
  ...
  Zap, ChevronDown, ChevronUp, Terminal, Server,
  AlertCircle, CornerDownRight,
} from 'lucide-react'
import {
  QUESTIONS, ROUTER_INTERFACES, TOPO_HIGHLIGHTS,
  LAB08_META, LAB08_SCENARIO, LAB08_COMPLETION,
  type Question, type MCQuestion, type CLIQuestion, type SeqQuestion,
} from '@/data/lab08Data'
...
const SEQ_COLORS: Record<string, string> = {
  'PC-A sends packet to its default gateway (R1)': 'bg-blue-100 text-blue-700 border-blue-200',
  'R1 checks its routing table for 192.168.2.0':   'bg-slate-100 text-slate-700 border-slate-200',
  'Router R1 forwards packet toward R2':           'bg-indigo-100 text-indigo-700 border-indigo-200',
  'Router R2 forwards packet to PC-B':             'bg-green-100 text-green-700 border-green-200',
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Linear — 2 Routers</span>
      </div>

      {/* Horizontal chain: PC-A — R1 ==== R2 — PC-B */}
      <div className="flex items-center justify-between gap-1 overflow-x-auto pb-1">

        {/* PC-A */}
        <div className={`flex flex-col items-center px-2.5 py-2 rounded-xl border-2 transition-all duration-300 min-w-[62px] ${nodeClass('PC-A', 'bg-blue-50 border-blue-200')}`}>
          <span className="text-lg">💻</span>
          <span className="text-xs font-bold text-gray-800">PC-A</span>
          <span className="text-xs text-gray-500">192.168.1.10</span>
        ...
        {/* R1 */}
        <div className={`flex flex-col items-center px-2.5 py-2 rounded-xl border-2 transition-all duration-300 min-w-[70px] ${nodeClass('R1', 'bg-slate-100 border-slate-300')}`}>
          <span className="text-lg">🌐</span>
          <span className="text-xs font-bold text-gray-800">R1</span>
          <span className="text-xs text-gray-500">G0/0: .1.1</span>
          <span className="text-xs text-gray-500">G0/1: .0.1</span>
        </div>

        {/* Dashed link R1 to R2 with label */}
        <div className="flex flex-col items-center flex-1 min-w-[52px]">
          <span className="text-xs text-gray-400 mb-0.5 whitespace-nowrap">10.0.0.0/30</span>
          <div className="w-full border-t-2 border-dashed border-slate-400" />
        </div>

        {/* R2 */}
        <div className={`flex flex-col items-center px-2.5 py-2 rounded-xl border-2 transition-all duration-300 min-w-[70px] ${nodeClass('R2', 'bg-slate-100 border-slate-300')}`}>
          <span className="text-lg">🌐</span>
          <span className="text-xs font-bold text-gray-800">R2</span>
          <span className="text-xs text-gray-500">G0/0: .0.2</span>
          <span className="text-xs text-gray-500">G0/1: .2.1</span>
        </div>
        ...
        {/* PC-B */}
        <div className={`flex flex-col items-center px-2.5 py-2 rounded-xl border-2 transition-all duration-300 min-w-[62px] ${nodeClass('PC-B', 'bg-blue-50 border-blue-200')}`}>
          <span className="text-lg">🖥️</span>
          <span className="text-xs font-bold text-gray-800">PC-B</span>
          <span className="text-xs text-gray-500">192.168.2.20</span>
          <span className="text-xs text-gray-400">/24</span>
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 grid grid-cols-2 gap-1.5 text-center">
        {[
          { label: 'PC Network', color: 'bg-blue-50 text-blue-700' },
          { label: 'Router Link', color: 'bg-slate-100 text-slate-600' },
        ].map(t => (
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          <Server className="w-4 h-4 text-blue-500" />
          <span className="font-semibold text-gray-800 text-sm">Device Details</span>
        </div>
          ...
          <div className="rounded-xl overflow-hidden border border-gray-200">
            <table className="w-full text-xs">
              <thead className="bg-blue-50">
                <tr>
                  <th className="text-left px-3 py-2 font-semibold text-blue-600">Router</th>
                  <th className="text-left px-3 py-2 font-semibold text-blue-600">Interface</th>
                  <th className="text-left px-3 py-2 font-semibold text-blue-600">IP</th>
                  <th className="text-left px-3 py-2 font-semibold text-blue-600 hidden sm:table-cell">Connects To</th>
                </tr>
              </thead>
              <tbody>
                {ROUTER_INTERFACES.map((row, i) => (
                  <tr key={i} className={`${row.router === 'R1' ? 'border-l-2 border-blue-400' : 'border-l-2 border-slate-400'} ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    <td className="px-3 py-2 font-bold text-gray-800">{row.router}</td>
                    <td className="px-3 py-2 font-mono text-gray-700">{row.interface}</td>
                    <td className="px-3 py-2 font-mono text-gray-700">{row.ip}</td>
                    <td className="px-3 py-2 text-gray-500 hidden sm:table-cell">{row.connects}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ScenarioCard() {
  const [open, setOpen] = useState(true)
  return (
    <div className="bg-gradient-to-br from-blue-50 to-slate-50 rounded-2xl border border-blue-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          <span className="text-lg">🔍</span>
          <span className="font-semibold text-blue-900">Mission Brief</span>
        </div>
        {open ? <ChevronUp className="w-4 h-4 text-blue-500" /> : <ChevronDown className="w-4 h-4 text-blue-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-blue-900">{LAB08_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-blue-700 uppercase tracking-wider mb-1.5">Users report:</p>
            <ul className="space-y-1">
              {LAB08_SCENARIO.reports.map(r => (
                <li key={r} className="text-sm text-blue-800 flex items-start gap-2">
          ...
          <div className="bg-white/70 rounded-xl p-3 border border-blue-100">
            <p className="text-sm text-blue-900 font-medium">{LAB08_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-blue-600">Lab 08</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1">
              <Star className="w-3 h-3 text-yellow-400" />{LAB08_META.difficulty}
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB08_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB08_META.estimatedTime}</span>
        ...
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">
            {LAB08_META.skillsTested.map(s => (
              <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5">
                <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />{s}
              </li>
            ))}
          </ul>
        </div>
    ...
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        if (state.submitted || state.revealed) {
          ...
          cls = 'border-blue-400 bg-blue-50 ring-2 ring-blue-200 cursor-pointer'
        }
        return (
          <button
            key={opt.key}
          ...
          >
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed
                ...
                : sel ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600'
            }`}>{opt.key}</span>
  ...
  return (
    <div className={`rounded-xl border p-4 ${revealed ? 'bg-blue-50 border-blue-200' : 'bg-red-50 border-red-200'}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className={`font-semibold ${revealed ? 'text-blue-700' : 'text-red-700'}`}>{revealed ? 'Here is the answer' : 'Not quite — try again or reveal'}</span>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 p-6 text-center">
          ...
          <p className="text-blue-100 text-sm mt-1">{LAB08_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB08_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">
              {LAB08_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">
                {LAB08_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB08_COMPLETION.nextLab.title}</p>
          ...
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm transition-all">
              Back to Labs
            </Link>
          </div>
        </div>
...
export default function Lab08Page() {
  ...
  const handleRetry = useCallback(() =>
    setQState(s => ({
      ...s, submitted: false, isCorrect: null, selected: null, cliInput: '',
  ...
  const handleReveal = useCallback(() =>
    setQState(s => ({
      ...s, submitted: true, revealed: true, isCorrect: false,
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab08', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => (
                <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-blue-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              <div className="flex items-start justify-between gap-3 mb-5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {currentQ.type === 'cli-input' && (
                    <span className="text-xs text-green-600 flex items-center gap-1"><Terminal className="w-3 h-3" />Type the Cisco IOS command</span>
                  )}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
                  Check Answer
                </button>
              )}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              ...
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => (
                  <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-blue-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB08_META.lessonsReinforced.map(l => (
