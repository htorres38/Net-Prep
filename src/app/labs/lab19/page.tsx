...
import {
  ...
  LAB19_META, LAB19_SCENARIO, LAB19_COMPLETION,
...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  'topology-reasoning': { label: 'Output Analysis',    color: 'bg-indigo-100 text-indigo-700' },
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, addr }: { name: string; icon: string; addr?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[100px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-fuchsia-300 bg-fuchsia-50'}`}>
      <span className="text-lg">{icon}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      {addr && <span className="text-xs font-mono text-fuchsia-700">{addr}</span>}
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Troubleshooting Lab</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <div className="flex gap-3 justify-center">
          <Node name="PC-A" icon="💻" addr="VLAN 10 .10.10" />
          <Node name="PC-B" icon="💻" addr="VLAN 20 .20.10" />
        </div>
        <div className="w-px h-3 bg-fuchsia-300" />
        <Node name="S1" icon="🔀" addr="trunk G0/1" />
        <div className="w-px h-3 bg-fuchsia-300" />
        <Node name="R1" icon="🌐" addr=".10.1 | .20.1 | .12.1" />
        <div className="w-px h-3 bg-fuchsia-300" />
        <Node name="R2" icon="🌐" addr=".12.2 | .30.1 | NAT" />
        <div className="w-px h-3 bg-fuchsia-300" />
        <Node name="Server" icon="🖥️" addr="192.168.30.10" />
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100 space-y-1">
        <div className="flex justify-between text-xs">
          <span className="text-fuchsia-600 font-semibold">R1↔R2 link</span>
          <span className="text-gray-400">10.0.12.0/30</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-fuchsia-600 font-semibold">NAT on R2</span>
          <span className="text-gray-400">Inside → 203.0.113.5</span>
        </div>
      </div>
    </div>
  )
...
// troubleshooting ref
function TroubleshootingReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">Troubleshooting Quick Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Symptom → Root Cause</p>
            {[
              { symptom: 'Local gateway OK, remote fails', cause: 'Routing / missing route' },
              { symptom: '169.254.x.x address',            cause: 'DHCP failure (APIPA)' },
              { symptom: 'Ping works, hostname fails',      cause: 'DNS failure' },
              { symptom: 'Local OK, no internet',           cause: 'NAT missing' },
              { symptom: 'OSPF state not FULL',             cause: 'Area / timer mismatch' },
              { symptom: 'Not-trunking on switch',          cause: 'switchport mode trunk' },
              { symptom: 'Port err-disabled',               cause: 'Port Security violation' },
            ].map(r => (
              <div key={r.symptom} className="rounded-lg px-3 py-1.5 mb-1 bg-fuchsia-50 border border-fuchsia-200">
                <span className="text-xs text-fuchsia-800"><span className="font-bold">{r.symptom}</span> → {r.cause}</span>
              </div>
            ))}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Key Verification Commands</p>
            <div className="bg-gray-900 rounded-lg p-2 space-y-1 text-xs font-mono text-green-400">
              <p>show ip route</p>
              <p>show ip ospf neighbor</p>
              <p>show interfaces trunk</p>
              <p>show vlan brief</p>
              <p>show ip nat translations</p>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-fuchsia-50 to-pink-50 rounded-2xl border border-fuchsia-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-fuchsia-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-fuchsia-500" /> : <ChevronDown className="w-4 h-4 text-fuchsia-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-fuchsia-900">{LAB19_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-fuchsia-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB19_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-fuchsia-800 flex items-start gap-2"><span className="text-pink-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-fuchsia-100">
            <p className="text-sm text-fuchsia-900 font-medium">{LAB19_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-fuchsia-600">Lab 19</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB19_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB19_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB19_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-fuchsia-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB19_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-fuchsia-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-fuchsia-500 to-pink-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-fuchsia-300 hover:bg-fuchsia-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-fuchsia-400 bg-fuchsia-50 ring-2 ring-fuchsia-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-fuchsia-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200', 'bg-pink-100 text-pink-700 border-pink-200', 'bg-purple-100 text-purple-700 border-purple-200', 'bg-rose-100 text-rose-700 border-rose-200']
  return (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-fuchsia-500 to-pink-600 p-6 text-center">
          <div className="text-5xl mb-2">🏆</div>
          <h2 className="text-2xl font-bold text-white">Lab Complete!</h2>
          <p className="text-fuchsia-100 text-sm mt-1">{LAB19_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-fuchsia-50 rounded-2xl p-4 text-center border border-fuchsia-100">
              <p className="text-3xl font-bold text-fuchsia-600">{score}/{total}</p>
              <p className="text-xs text-fuchsia-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB19_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB19_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB19_COMPLETION.reviewIfNeeded.map(l => (
                <li key={l.id} className="text-xs text-amber-800 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0" />Lessons {l.id}: {l.title}</li>
              ))}</ul>
            </div>
          )}
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Next Suggested Lab</p>
            <p className="text-sm font-semibold text-gray-800">{LAB19_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab19Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab19', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-fuchsia-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 section-enter">
              <div className="flex items-start justify-between gap-3 mb-5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {currentQ.type === 'topology-reasoning' && <span className="text-xs text-indigo-600 flex items-center gap-1"><AlertCircle className="w-3 h-3" />Analyze the output</span>}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-fuchsia-600 hover:bg-fuchsia-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-fuchsia-600">{score} correct</span>
              </div>
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-fuchsia-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB19_META.lessonsReinforced.map(l => (
              <span key={l.id} className="text-xs font-medium text-gray-600 bg-gray-100 rounded-lg px-3 py-1.5">Lessons {l.id}: {l.title}</span>
            ))}
          </div>
        </div>
      </div>
