...
import {
  ...
  LAB15_META, LAB15_SCENARIO, LAB15_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, sub }: { name: string; icon: string; sub?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[110px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-violet-300 bg-violet-50'}`}>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Management Protocols</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <Node name="NTP Server" icon="🕐" sub="Stratum 1" />
        <div className="flex flex-col items-center gap-0.5 my-0.5">
          <div className="w-px h-3 bg-violet-300" />
          <span className="text-xs font-semibold text-violet-600 bg-violet-50 border border-violet-200 rounded px-1.5 py-0.5">NTP Sync</span>
          <div className="w-px h-3 bg-violet-300" />
        </div>
        <Node name="Router" icon="🌐" sub="Stratum 2" />
        <div className="flex flex-col items-center gap-0.5 my-0.5">
          <div className="w-px h-3 bg-violet-300" />
          <span className="text-xs font-semibold text-violet-600 bg-violet-50 border border-violet-200 rounded px-1.5 py-0.5">CDP / LLDP</span>
          <div className="w-px h-3 bg-violet-300" />
        </div>
        <Node name="Switch" icon="🔀" sub="Syslog / SNMP" />
        <div className="w-px h-3 bg-violet-300" />
        <Node name="PC" icon="💻" sub="Management host" />
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100">
        <div className="flex justify-center gap-4">
          <div className="text-center">
            <p className="text-xs font-semibold text-violet-600">NMS</p>
            <p className="text-xs text-gray-400">SNMP Manager</p>
          </div>
          <div className="text-center">
            <p className="text-xs font-semibold text-violet-600">Syslog</p>
            <p className="text-xs text-gray-400">Central log server</p>
          </div>
        </div>
      </div>
    </div>
...
// mgmt ref
function MgmtReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">Management Protocols Quick Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Syslog Severity Levels</p>
            {[
              { level: '0', name: 'Emergency',   color: 'bg-red-100 text-red-700',       desc: 'System unusable' },
              { level: '1', name: 'Alert',        color: 'bg-red-50 text-red-600',        desc: 'Immediate action needed' },
              { level: '2', name: 'Critical',     color: 'bg-orange-100 text-orange-700', desc: 'Critical conditions' },
              { level: '3', name: 'Error',        color: 'bg-orange-50 text-orange-600',  desc: 'Error conditions' },
              { level: '4', name: 'Warning',      color: 'bg-yellow-100 text-yellow-700', desc: 'Warning conditions' },
              { level: '5', name: 'Notice',       color: 'bg-blue-50 text-blue-700',      desc: 'Normal but significant' },
              { level: '6', name: 'Info',         color: 'bg-green-50 text-green-700',    desc: 'Informational' },
              { level: '7', name: 'Debug',        color: 'bg-gray-50 text-gray-600',      desc: 'Debug-level' },
            ].map(s => (
              <div key={s.level} className={`rounded-lg px-3 py-1 mb-0.5 ${s.color} border border-current border-opacity-20 flex items-center gap-2`}>
                <span className="text-xs font-bold w-4">{s.level}</span>
                <span className="text-xs font-semibold w-20">{s.name}</span>
                <span className="text-xs opacity-70">— {s.desc}</span>
              </div>
            ))}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Protocol Comparison</p>
            <div className="bg-violet-50 border border-violet-200 rounded-lg p-2 space-y-1 text-xs">
              <p className="text-violet-800"><span className="font-bold">CDP</span> — Cisco proprietary, Layer 2 discovery</p>
              <p className="text-violet-800"><span className="font-bold">LLDP</span> — IEEE 802.1AB, vendor-neutral</p>
              <p className="text-violet-800"><span className="font-bold">NTP</span> — UDP port 123, clock sync</p>
              <p className="text-violet-800"><span className="font-bold">SNMP</span> — UDP 161 (query), 162 (trap)</p>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-2xl border border-violet-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-violet-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-violet-500" /> : <ChevronDown className="w-4 h-4 text-violet-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-violet-900">{LAB15_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-violet-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB15_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-violet-800 flex items-start gap-2"><span className="text-red-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-violet-100">
            <p className="text-sm text-violet-900 font-medium">{LAB15_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-violet-600">Lab 15</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB15_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB15_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB15_META.estimatedTime}</span>
        ...
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB15_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-violet-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-violet-500 to-purple-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        } else if (sel) cls = 'border-violet-400 bg-violet-50 ring-2 ring-violet-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-violet-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-violet-100 text-violet-700 border-violet-200', 'bg-purple-100 text-purple-700 border-purple-200', 'bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200', 'bg-indigo-100 text-indigo-700 border-indigo-200']
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-violet-500 to-purple-600 p-6 text-center">
          ...
          <p className="text-violet-100 text-sm mt-1">{LAB15_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB15_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB15_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB15_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB15_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab15Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab15', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-violet-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-violet-600 hover:bg-violet-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              ...
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-violet-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB15_META.lessonsReinforced.map(l => (
