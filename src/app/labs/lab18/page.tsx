...
import {
  ...
  LAB18_META, LAB18_SCENARIO, LAB18_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, addr }: { name: string; icon: string; addr?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[110px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-indigo-300 bg-indigo-50'}`}>
      <span className="text-lg">{icon}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      {addr && <span className="text-xs font-mono text-indigo-700">{addr}</span>}
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Automation Lab</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <Node name="Automation Server" icon="🤖" addr="Ansible / API" />
        <div className="w-px h-3 bg-indigo-300" />
        <div className="flex gap-3 justify-center">
          <Node name="Router" icon="🌐" addr="REST API" />
          <Node name="Switch" icon="🔀" addr="SSH" />
          <Node name="AP" icon="📡" addr="CAPWAP" />
        </div>
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100 space-y-1">
        <div className="flex justify-between text-xs">
          <span className="text-indigo-600 font-semibold">REST API</span>
          <span className="text-gray-400">HTTP GET/POST/PUT/DELETE</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-indigo-600 font-semibold">Ansible</span>
          <span className="text-gray-400">YAML playbooks via SSH</span>
        </div>
      </div>
    </div>
  )
...
// automation ref
function AutomationReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">Automation Quick Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Data Formats</p>
            {[
              { fmt: 'JSON',  color: 'bg-indigo-50 text-indigo-700',  desc: 'Key-value pairs in { } — REST API standard' },
              { fmt: 'XML',   color: 'bg-purple-50 text-purple-700',  desc: '<tag>value</tag> — used in NETCONF' },
              { fmt: 'YAML',  color: 'bg-violet-50 text-violet-700',  desc: 'Indentation-based — Ansible playbooks' },
            ].map(r => (
              <div key={r.fmt} className={`rounded-lg px-3 py-1.5 mb-1 ${r.color} border border-current border-opacity-20`}>
                <span className="text-xs font-bold">{r.fmt}</span>
                <span className="text-xs opacity-80 ml-2">— {r.desc}</span>
              </div>
            ))}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">REST HTTP Methods</p>
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-2 space-y-1 text-xs text-indigo-800">
              <p><span className="font-bold">GET</span> — retrieve (read-only)</p>
              <p><span className="font-bold">POST</span> — create new resource</p>
              <p><span className="font-bold">PUT</span> — update/replace resource</p>
              <p><span className="font-bold">DELETE</span> — remove resource</p>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Config Management Tools</p>
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-2 space-y-1 text-xs text-gray-700">
              <p><span className="font-bold">Ansible</span> — agentless, YAML playbooks, SSH</p>
              <p><span className="font-bold">Puppet</span> — agent-based, DSL manifests</p>
              <p><span className="font-bold">Chef</span> — agent-based, Ruby cookbooks</p>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-50 to-violet-50 rounded-2xl border border-indigo-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-indigo-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-indigo-500" /> : <ChevronDown className="w-4 h-4 text-indigo-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-indigo-900">{LAB18_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB18_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-indigo-800 flex items-start gap-2"><span className="text-violet-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-indigo-100">
            <p className="text-sm text-indigo-900 font-medium">{LAB18_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-indigo-600">Lab 18</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB18_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB18_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB18_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-indigo-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB18_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-indigo-500 to-violet-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
        ...
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-indigo-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-indigo-100 text-indigo-700 border-indigo-200', 'bg-violet-100 text-violet-700 border-violet-200', 'bg-purple-100 text-purple-700 border-purple-200', 'bg-blue-100 text-blue-700 border-blue-200']
  return (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-indigo-500 to-violet-600 p-6 text-center">
          <div className="text-5xl mb-2">🏆</div>
          <h2 className="text-2xl font-bold text-white">Lab Complete!</h2>
          <p className="text-indigo-100 text-sm mt-1">{LAB18_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-indigo-50 rounded-2xl p-4 text-center border border-indigo-100">
              ...
              <p className="text-xs text-indigo-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB18_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB18_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB18_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB18_COMPLETION.nextLab.title}</p>
          </div>
...
export default function Lab18Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab18', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-indigo-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              ...
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-indigo-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB18_META.lessonsReinforced.map(l => (
