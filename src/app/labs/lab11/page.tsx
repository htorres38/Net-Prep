...
import {
  ...
  LAB11_META, LAB11_SCENARIO, LAB11_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const SwitchNode = ({ name, label, isTop }: { name: string; label: string; isTop?: boolean }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[90px]
      ${hi.has(name) ? nodeClass(name) : isTop ? 'border-emerald-300 bg-emerald-50' : 'border-green-300 bg-green-50'}`}>
      <span className="text-lg">🔀</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-md mt-0.5 ${isTop ? 'bg-emerald-100 text-emerald-700' : 'bg-green-100 text-green-700'}`}>{label}</span>
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Redundant Switches</span>
      </div>

      <div className="flex flex-col items-center gap-2">
        {/* Switch A at top */}
        <SwitchNode name="Switch A" label="Root Candidate" isTop />

        {/* Lines from A down to B and C */}
        <div className="relative w-full flex justify-center" style={{ height: '24px' }}>
          <svg width="100%" height="24" style={{ overflow: 'visible' }}>
            <line x1="50%" y1="0" x2="28%" y2="100%" stroke="#6ee7b7" strokeWidth="1.5" />
            <line x1="50%" y1="0" x2="72%" y2="100%" stroke="#6ee7b7" strokeWidth="1.5" />
          </svg>
        </div>

        {/* Switch B and C */}
        <div className="flex items-center gap-4 w-full justify-center">
          <SwitchNode name="Switch B" label="Non-Root" />
          <div className="flex flex-col items-center gap-0.5">
            <div className="w-8 border-t-2 border-dashed border-green-400" />
            <span className="text-xs text-green-600 font-semibold bg-green-50 border border-green-200 rounded px-1 py-0.5">Loop Link</span>
          </div>
          <SwitchNode name="Switch C" label="Non-Root" />
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 space-y-1.5">
        {[
          { dot: 'bg-emerald-400', label: 'STP active (forwarding)' },
          { dot: 'bg-red-400',     label: 'STP blocked (loop prevention)' },
        ].map(t => (
...
// stp ref
function STPReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-gray-800">STP Port Roles & States</span>
        </div>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-2">
          <p className="text-xs text-gray-500 mb-2">Port roles and states for reference:</p>
          {[
            { role: 'Root',        state: 'FWD', color: 'bg-emerald-50 text-emerald-700 border-emerald-200', desc: 'Best path to root bridge' },
            { role: 'Designated',  state: 'FWD', color: 'bg-blue-50 text-blue-700 border-blue-200',     desc: 'Forwarding port on each segment' },
            { role: 'Alternate',   state: 'BLK', color: 'bg-red-50 text-red-700 border-red-200',       desc: 'Blocked redundant path' },
          ].map(r => (
            <div key={r.role} className={`rounded-lg border px-3 py-2 ${r.color}`}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold">{r.role} Port — {r.state}</span>
              </div>
              <span className="text-xs opacity-80">{r.desc}</span>
            </div>
          ))}
          <div className="mt-2 pt-2 border-t border-gray-100">
            <p className="text-xs font-semibold text-gray-500 mb-1">EtherChannel Protocols</p>
            <div className="flex gap-2">
              <span className="text-xs bg-blue-100 text-blue-700 rounded px-2 py-1">LACP — IEEE Open Standard</span>
              <span className="text-xs bg-orange-100 text-orange-700 rounded px-2 py-1">PAgP — Cisco Only</span>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl border border-emerald-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          <span className="text-lg">🔍</span>
          <span className="font-semibold text-emerald-800">Mission Brief</span>
        </div>
        {open ? <ChevronUp className="w-4 h-4 text-emerald-500" /> : <ChevronDown className="w-4 h-4 text-emerald-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-emerald-900">{LAB11_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wider mb-1.5">Users report:</p>
            <ul className="space-y-1">{LAB11_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-emerald-800 flex items-start gap-2">
                <span className="text-red-400 mt-0.5">!</span>{r}
              </li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-emerald-100">
            <p className="text-sm text-emerald-900 font-medium">{LAB11_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-emerald-600">Lab 11</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1">
              <Star className="w-3 h-3 text-yellow-400" />{LAB11_META.difficulty}
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB11_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB11_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-emerald-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB11_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5">
              <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />{s}
            </li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-emerald-500 to-green-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-emerald-300 hover:bg-emerald-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-emerald-400 bg-emerald-50 ring-2 ring-emerald-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              ...
              : sel ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-emerald-100 text-emerald-700 border-emerald-200', 'bg-green-100 text-green-700 border-green-200', 'bg-teal-100 text-teal-700 border-teal-200', 'bg-cyan-100 text-cyan-700 border-cyan-200']
  return (
      ...
      {state.seqOrder.map((item, idx) => {
        ...
        return (
          <div key={item} className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${done ? correct ? 'border-green-400 bg-green-50' : 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
            ...
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border flex-1 ${color}`}>{item}</span>
        ...
        <div className="mt-2 p-3 rounded-xl bg-gray-50 border border-gray-200">
          ...
          <ol className="space-y-1">{question.correctOrder.map((item, i) => (
            <li key={item} className="text-sm text-gray-700 flex items-center gap-2">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border ${COLORS[i % COLORS.length]}`}>{item}</span>
            </li>
          ))}</ol>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-emerald-500 to-green-600 p-6 text-center">
          ...
          <p className="text-emerald-100 text-sm mt-1">{LAB11_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-emerald-50 rounded-2xl p-4 text-center border border-emerald-100">
              <p className="text-3xl font-bold text-emerald-600">{score}/{total}</p>
              <p className="text-xs text-emerald-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB11_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB11_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB11_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB11_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab11Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab11', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => (
                <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-emerald-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
                  Check Answer
                </button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-emerald-600">{score} correct</span>
              </div>
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => (
                  <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-emerald-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB11_META.lessonsReinforced.map(l => (
