...
import {
  ...
  LAB14_META, LAB14_SCENARIO, LAB14_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, addr, sub }: { name: string; icon: string; addr?: string; sub?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[110px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-orange-300 bg-orange-50'}`}>
      <span className="text-lg">{icon}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      {addr && <span className="text-xs font-mono text-orange-700">{addr}</span>}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">DHCP / DNS / NAT</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <Node name="PC1" icon="💻" addr="192.168.1.x (DHCP)" sub="Private IP" />
        <div className="w-px h-3 bg-orange-300" />
        <Node name="Switch" icon="🔀" sub="Layer 2" />
        <div className="w-px h-3 bg-orange-300" />
        <Node name="Router" icon="🌐" addr="LAN: 192.168.1.1" sub="WAN: 203.0.113.5" />
        <div className="flex flex-col items-center gap-0.5 my-0.5">
          <div className="w-px h-3 bg-orange-300" />
          <span className="text-xs font-semibold text-orange-600 bg-orange-50 border border-orange-200 rounded px-1.5 py-0.5">NAT / Internet</span>
          <div className="w-px h-3 bg-orange-300" />
        </div>
        <Node name="DNS Server" icon="🖥️" addr="8.8.8.8" sub="Public DNS" />
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100">
        <p className="text-xs text-gray-400 text-center">Small office with DHCP, DNS, and PAT</p>
      </div>
    </div>
  )
}
...
// dhcp/dns/nat ref
function DHCPReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">IP Services Quick Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">DHCP DORA Process</p>
            {[
              { step: 'D — Discover', color: 'bg-orange-50 text-orange-700',  desc: 'Client broadcasts to find server' },
              { step: 'O — Offer',    color: 'bg-amber-50 text-amber-700',    desc: 'Server offers an IP address' },
              { step: 'R — Request',  color: 'bg-yellow-50 text-yellow-700',  desc: 'Client requests the offered IP' },
              { step: 'A — Acknowledge', color: 'bg-green-50 text-green-700', desc: 'Server confirms the assignment' },
            ].map(s => (
              <div key={s.step} className={`rounded-lg px-3 py-1.5 mb-1 ${s.color} border border-current border-opacity-20`}>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">NAT Address Types</p>
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 space-y-1 text-xs text-orange-800">
              <p><span className="font-bold">Inside local</span> — private IP of internal device</p>
              <p><span className="font-bold">Inside global</span> — public IP after NAT</p>
              <p><span className="font-bold">Outside global</span> — destination IP</p>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">APIPA Range</p>
            <div className="bg-red-50 border border-red-200 rounded-lg p-2 text-xs font-mono text-red-800">
              169.254.0.0/16 — DHCP failure indicator
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl border border-orange-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-orange-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-orange-500" /> : <ChevronDown className="w-4 h-4 text-orange-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-orange-900">{LAB14_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-orange-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB14_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-orange-800 flex items-start gap-2"><span className="text-red-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-orange-100">
            <p className="text-sm text-orange-900 font-medium">{LAB14_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-orange-600">Lab 14</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB14_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB14_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB14_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-orange-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB14_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-orange-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-orange-500 to-amber-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-orange-300 hover:bg-orange-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-orange-400 bg-orange-50 ring-2 ring-orange-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
      ...
      {state.seqOrder.map((item, idx) => {
        ...
        return (
          <div key={item} className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${done ? correct ? 'border-green-400 bg-green-50' : 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
            ...
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border flex-1 ${SEQ_COLORS[idx % SEQ_COLORS.length]}`}>{item}</span>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-orange-500 to-amber-600 p-6 text-center">
          <div className="text-5xl mb-2">🏆</div>
          <h2 className="text-2xl font-bold text-white">Lab Complete!</h2>
          <p className="text-orange-100 text-sm mt-1">{LAB14_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-orange-50 rounded-2xl p-4 text-center border border-orange-100">
              <p className="text-3xl font-bold text-orange-600">{score}/{total}</p>
              <p className="text-xs text-orange-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB14_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB14_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB14_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB14_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab14Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab14', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-orange-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-orange-600 hover:bg-orange-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              ...
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-orange-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB14_META.lessonsReinforced.map(l => (
