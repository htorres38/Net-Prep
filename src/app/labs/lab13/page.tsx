...
import {
  ...
  LAB13_META, LAB13_SCENARIO, LAB13_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, addr, sub }: { name: string; icon: string; addr?: string; sub?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[110px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-cyan-300 bg-cyan-50'}`}>
      <span className="text-lg">{icon}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      {addr && <span className="text-xs font-mono text-cyan-700">{addr}</span>}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">IPv6 Network</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <Node name="PC A" icon="💻" addr="2001:DB8:1:1::10" sub="FE80::10 (link-local)" />
        <div className="flex flex-col items-center gap-0.5 my-0.5">
          <div className="w-px h-3 bg-cyan-300" />
          <span className="text-xs font-semibold text-cyan-600 bg-cyan-50 border border-cyan-200 rounded px-1.5 py-0.5">RS / RA</span>
          <div className="w-px h-3 bg-cyan-300" />
        </div>
        <Node name="Router" icon="🌐" addr="2001:DB8:1:1::1" sub="Area boundary" />
        <div className="flex flex-col items-center gap-0.5 my-0.5">
          <div className="w-px h-3 bg-cyan-300" />
          <span className="text-xs font-semibold text-cyan-600 bg-cyan-50 border border-cyan-200 rounded px-1.5 py-0.5">IPv6 Link</span>
          <div className="w-px h-3 bg-cyan-300" />
        </div>
        <Node name="Server" icon="🖥️" addr="2001:DB8:2:1::10" sub="Global unicast" />
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100">
        <p className="text-xs text-gray-400 text-center">IPv6 dual-stack transition network</p>
      </div>
    </div>
  )
}
...
// ipv6 ref
function IPv6ReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">IPv6 Quick Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Address Types</p>
            {[
              { range: '2000::/3',   type: 'Global Unicast',  color: 'bg-green-50 text-green-700',  desc: 'Internet-routable' },
              { range: 'FE80::/10', type: 'Link-Local',      color: 'bg-blue-50 text-blue-700',    desc: 'Local segment only' },
              { range: 'FF00::/8',  type: 'Multicast',       color: 'bg-purple-50 text-purple-700', desc: 'Group delivery' },
              { range: '::1/128',   type: 'Loopback',        color: 'bg-gray-50 text-gray-700',    desc: 'Local device only' },
            ].map(r => (
              <div key={r.range} className={`rounded-lg px-3 py-1.5 mb-1 ${r.color} border border-current border-opacity-20`}>
                <span className="text-xs font-bold font-mono">{r.range}</span>
                <span className="text-xs font-semibold ml-2">— {r.type}</span>
                <span className="text-xs opacity-70 ml-1">({r.desc})</span>
              </div>
            ))}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Key Multicast Addresses</p>
            <div className="bg-cyan-50 border border-cyan-200 rounded-lg p-2 space-y-1">
              <p className="text-xs font-mono text-cyan-800">FF02::1 — All nodes (local link)</p>
              <p className="text-xs font-mono text-cyan-800">FF02::2 — All routers (local link)</p>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Shortening Rules</p>
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-2 space-y-1 text-xs text-gray-700">
              <p>1. Drop leading zeros in each group</p>
              <p>2. Replace longest consecutive all-zero groups with ::</p>
              <p>3. :: can only appear once</p>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-cyan-50 to-teal-50 rounded-2xl border border-cyan-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-cyan-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-cyan-500" /> : <ChevronDown className="w-4 h-4 text-cyan-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-cyan-900">{LAB13_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-cyan-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB13_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-cyan-800 flex items-start gap-2"><span className="text-red-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-cyan-100">
            <p className="text-sm text-cyan-900 font-medium">{LAB13_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-cyan-600">Lab 13</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB13_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB13_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB13_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-cyan-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB13_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-cyan-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-cyan-500 to-teal-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-cyan-300 hover:bg-cyan-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-cyan-400 bg-cyan-50 ring-2 ring-cyan-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-cyan-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-cyan-100 text-cyan-700 border-cyan-200', 'bg-teal-100 text-teal-700 border-teal-200', 'bg-sky-100 text-sky-700 border-sky-200', 'bg-blue-100 text-blue-700 border-blue-200']
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-cyan-500 to-teal-600 p-6 text-center">
          ...
          <p className="text-cyan-100 text-sm mt-1">{LAB13_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-cyan-50 rounded-2xl p-4 text-center border border-cyan-100">
              <p className="text-3xl font-bold text-cyan-600">{score}/{total}</p>
              <p className="text-xs text-cyan-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB13_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB13_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB13_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB13_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab13Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab13', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-cyan-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-cyan-600 hover:bg-cyan-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-cyan-600">{score} correct</span>
              </div>
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-cyan-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB13_META.lessonsReinforced.map(l => (
