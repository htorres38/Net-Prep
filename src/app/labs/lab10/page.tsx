...
import {
  ...
  CornerDownRight, Layers, Network,
} from 'lucide-react'
import {
  QUESTIONS, ROUTER_ON_STICK_STEPS, SVI_STEPS, TOPO_HIGHLIGHTS,
  LAB10_META, LAB10_SCENARIO, LAB10_COMPLETION,
...
const SEQ_COLORS: Record<string, string> = {
  'PC-A sends frame to default gateway':   'bg-purple-100 text-purple-700 border-purple-200',
  'Switch tags frame with VLAN 10 ID':     'bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200',
  'Router receives tagged frame on G0/0.10': 'bg-blue-100 text-blue-700 border-blue-200',
  'Router routes packet to VLAN 20':       'bg-green-100 text-green-700 border-green-200',
}
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const PCNode = ({ name, vlan, port, isVlan10 }: { name: string; vlan: string; port: string; isVlan10: boolean }) => (
    <div className={`flex flex-col items-center px-2.5 py-2 rounded-xl border-2 transition-all duration-300 min-w-[72px]
      ${hi.has(name) ? nodeClass(name) : isVlan10 ? 'border-blue-300 bg-blue-50' : 'border-orange-300 bg-orange-50'}`}>
      <span className="text-lg">{isVlan10 ? '💻' : '🖥️'}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-md mt-0.5 ${isVlan10 ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>{vlan}</span>
      <span className="text-xs text-gray-400">{port}</span>
    </div>
  )

  const SwitchNode = () => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[90px]
      ${hi.has('Switch S1') ? nodeClass('Switch S1') : 'border-purple-300 bg-purple-50'}`}>
      <span className="text-lg">🔀</span>
      <span className="text-xs font-bold text-gray-800">Switch S1</span>
      <span className="text-xs text-purple-600 font-medium">VLAN 10 + 20</span>
    </div>
  )

  const RouterNode = () => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[130px]
      ${hi.has('Router R1') ? nodeClass('Router R1') : 'border-fuchsia-300 bg-fuchsia-50'}`}>
      <span className="text-lg">🌐</span>
      <span className="text-xs font-bold text-gray-800">Router R1</span>
      <span className="text-xs text-fuchsia-600 font-mono">G0/0.10: .10.1</span>
      <span className="text-xs text-pink-600 font-mono">G0/0.20: .20.1</span>
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Router-on-a-Stick</span>
      </div>

      <div className="flex flex-col items-center gap-0">
        {/* PCs row */}
        <div className="flex justify-center gap-6">
          <PCNode name="PC-A" vlan="VLAN 10" port="Fa0/3" isVlan10={true} />
          <PCNode name="PC-B" vlan="VLAN 20" port="Fa0/5" isVlan10={false} />
        </div>

        {/* Converging lines to switch */}
        ...
        <div className="flex flex-col items-center gap-0.5 my-1">
          ...
          <span className="text-xs font-semibold text-purple-600 bg-purple-50 border border-purple-200 rounded px-1.5 py-0.5">G0/1 — 802.1Q Trunk</span>
          <div className="w-px h-3 bg-purple-300" />
        </div>
      ...
      {/* Legend */}
      <div className="mt-4 pt-3 border-t border-gray-100 grid grid-cols-2 gap-1.5">
        {[
          { label: 'VLAN 10 – Eng', color: 'bg-blue-50 text-blue-700' },
          { label: 'VLAN 20 – Sales', color: 'bg-orange-50 text-orange-700' },
        ].map(t => (
          <div key={t.label} className={`rounded-lg px-2 py-1.5 text-center ${t.color}`}>
...
// config ref
function ConfigReferencePanel() {
  const [open, setOpen] = useState(false)
  const [tab, setTab] = useState<'roas' | 'svi'>('roas')
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-800 text-sm">Config Reference</span>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setTab('roas')}
              className={`flex-1 text-xs font-semibold py-1.5 rounded-lg transition-all ${tab === 'roas' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
              Router-on-a-Stick
            </button>
            <button onClick={() => setTab('svi')}
              className={`flex-1 text-xs font-semibold py-1.5 rounded-lg transition-all ${tab === 'svi' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
              L3 Switch SVIs
            </button>
          </div>
          ...
          {tab === 'roas' && (
            <div className="space-y-1.5">
              <p className="text-xs text-gray-500 mb-2">Full Router-on-a-Stick configuration:</p>
              {ROUTER_ON_STICK_STEPS.map((step, i) => (
                <div key={i} className={`rounded-lg border px-3 py-2 ${step.color}`}>
                  <div className="flex items-center justify-between mb-0.5">
                    <span className="text-xs font-bold">{step.label}</span>
                    <span className="text-xs font-mono opacity-60">{step.context}</span>
                  </div>
                  <span className="text-xs font-mono">{step.command}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {tab === 'svi' && (
            <div className="space-y-1.5">
              <p className="text-xs text-gray-500 mb-2">Layer 3 switch SVI configuration:</p>
              {SVI_STEPS.map((step, i) => (
                <div key={i} className="rounded-lg border border-purple-200 bg-purple-50 px-3 py-2">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className="text-xs font-bold text-purple-700">{step.label}</span>
                    <span className="text-xs font-mono text-purple-500">{step.context}</span>
                  </div>
                  <span className="text-xs font-mono text-purple-800">{step.command}</span>
                </div>
              ))}
              <p className="text-xs text-gray-400 italic mt-2">SVIs eliminate the need for an external router.</p>
            </div>
          )}
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 rounded-2xl border border-purple-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          <span className="text-lg">🔍</span>
          <span className="font-semibold text-purple-800">Mission Brief</span>
        </div>
        {open ? <ChevronUp className="w-4 h-4 text-purple-500" /> : <ChevronDown className="w-4 h-4 text-purple-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-purple-900">{LAB10_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-purple-700 uppercase tracking-wider mb-1.5">Users report:</p>
            <ul className="space-y-1">{LAB10_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-purple-800 flex items-start gap-2">
                <span className="text-red-400 mt-0.5">!</span>{r}
              </li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-purple-100">
            <p className="text-sm text-purple-900 font-medium">{LAB10_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-purple-600">Lab 10</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1">
              <Star className="w-3 h-3 text-yellow-400" />{LAB10_META.difficulty}
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB10_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB10_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-purple-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB10_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5">
              <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-purple-400 shrink-0" />{s}
            </li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-purple-500 to-fuchsia-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-purple-300 hover:bg-purple-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-purple-400 bg-purple-50 ring-2 ring-purple-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              ...
              : sel ? 'bg-purple-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
  ...
  return (
    <div className="space-y-2">
      {state.seqOrder.map((item, idx) => {
        ...
        const seqColor = SEQ_COLORS[item] ?? 'bg-gray-100 text-gray-600 border-gray-200'
        return (
          <div key={item} className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${done ? correct ? 'border-green-400 bg-green-50' : 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
            ...
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border flex-1 ${seqColor}`}>{item}</span>
        ...
        <div className="mt-2 p-3 rounded-xl bg-gray-50 border border-gray-200">
          ...
          <ol className="space-y-1">{question.correctOrder.map((item, i) => (
            <li key={item} className="text-sm text-gray-700 flex items-center gap-2">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border ${SEQ_COLORS[item] ?? 'bg-gray-100 text-gray-600 border-gray-200'}`}>{item}</span>
            </li>
          ))}</ol>
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-purple-500 to-fuchsia-600 p-6 text-center">
          <div className="text-5xl mb-2">🏆</div>
          <h2 className="text-2xl font-bold text-white">Lab Complete!</h2>
          <p className="text-purple-100 text-sm mt-1">{LAB10_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-purple-50 rounded-2xl p-4 text-center border border-purple-100">
              <p className="text-3xl font-bold text-purple-600">{score}/{total}</p>
              <p className="text-xs text-purple-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB10_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB10_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB10_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB10_COMPLETION.nextLab.title}</p>
          ...
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab10Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab10', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => (
                <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-purple-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 section-enter">
              <div className="flex items-start justify-between gap-3 mb-5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {currentQ.type === 'conceptual' && <span className="text-xs text-purple-600 flex items-center gap-1"><Layers className="w-3 h-3" />Layer 2 vs Layer 3</span>}
                </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-purple-600 hover:bg-purple-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
                  Check Answer
                </button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-purple-600">{score} correct</span>
              </div>
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => (
                  <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-purple-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB10_META.lessonsReinforced.map(l => (
