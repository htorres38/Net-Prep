...
import {
  ...
  AlertCircle, CornerDownRight, Table2, Hash,
} from 'lucide-react'
import {
  QUESTIONS, SUBNET_BLOCKS, TOPO_HIGHLIGHTS,
  LAB06_META, LAB06_SCENARIO, LAB06_COMPLETION,
...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  'subnet-identification':{ label: 'Subnet Identification',color: 'bg-amber-100 text-amber-700' },
  'calculation':          { label: 'Calculation',          color: 'bg-orange-900 text-orange-200' },
...
const SEQ_COLORS: Record<string, string> = {
  'PC checks if destination is local or remote': 'bg-blue-100 text-blue-700 border-blue-200',
  'PC sends packet to default gateway':          'bg-amber-100 text-amber-700 border-amber-200',
  'Router receives the frame':                   'bg-orange-100 text-orange-700 border-orange-200',
  'Router forwards packet to destination':       'bg-green-100 text-green-700 border-green-200',
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">192.168.1.0/26</span>
      </div>

      <div className="flex flex-col items-center gap-0">
        {/* Subnet 0 — PC-A and PC-B */}
        <div className="flex justify-center gap-6 w-full">
          <Node name="PC-A" emoji="💻" ip=".10/26" label="Subnet 0" baseClass="bg-amber-50 border-amber-200" />
          <Node name="PC-B" emoji="🖥️" ip=".50/26" label="Subnet 0" baseClass="bg-amber-50 border-amber-200" />
        </div>

        {/* Lines converging to switch */}
        <div className="relative w-full flex justify-center" style={{ height: '28px' }}>
          <svg width="100%" height="28" className="absolute inset-0" style={{ overflow: 'visible' }}>
            <line x1="32%" y1="0" x2="50%" y2="100%" stroke="#d97706" strokeWidth="1.5" strokeOpacity="0.4" />
            <line x1="68%" y1="0" x2="50%" y2="100%" stroke="#d97706" strokeWidth="1.5" strokeOpacity="0.4" />
          </svg>
        </div>

        {/* Switch */}
        <Node name="Switch" emoji="🔀" baseClass="bg-gray-50 border-gray-200" />
        <div className="w-px h-4 bg-gray-300" />

        {/* Router with interface labels */}
        <Node name="Router" emoji="🌐" ip="G0/0: .1/26" label="G0/1: .65/26" baseClass="bg-blue-50 border-blue-200" />
        <div className="w-px h-4 bg-orange-300" />

        {/* Subnet 1 — PC-C */}
        <Node name="PC-C" emoji="🖥️" ip=".70/26" label="Subnet 1" baseClass="bg-orange-50 border-orange-200" />
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 grid grid-cols-2 gap-1.5 text-center">
        <div className="rounded-lg px-2 py-1.5 bg-amber-50 text-amber-700">
          <span className="text-xs font-semibold">Subnet 0: 192.168.1.0/26</span>
          <p className="text-xs opacity-70">(0–63)</p>
        </div>
        <div className="rounded-lg px-2 py-1.5 bg-orange-50 text-orange-700">
          <span className="text-xs font-semibold">Subnet 1: 192.168.1.64/26</span>
          <p className="text-xs opacity-70">(64–127)</p>
        </div>
      </div>
    </div>
  )
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-800 text-sm">Subnet Blocks</span>
        </div>
          ...
          <div className="rounded-xl overflow-hidden border border-amber-100">
            <table className="w-full text-xs">
              <thead className="bg-amber-50">
                <tr>
                  <th className="text-left px-3 py-2 font-semibold text-amber-700">Network</th>
                  <th className="text-left px-3 py-2 font-semibold text-amber-700">Range</th>
                  <th className="text-left px-3 py-2 font-semibold text-amber-700">Broadcast</th>
                  <th className="text-right px-3 py-2 font-semibold text-amber-700">Hosts</th>
                </tr>
              </thead>
              <tbody>
                {SUBNET_BLOCKS.map((row, i) => (
                  <tr key={row.network} className={i % 2 === 0 ? 'bg-white' : 'bg-amber-50/40'}>
                    <td className="px-3 py-2 font-mono font-bold text-amber-700">{row.network}</td>
                    <td className="px-3 py-2 font-mono text-gray-700">{row.range}</td>
                    <td className="px-3 py-2 font-mono text-gray-600">{row.broadcast}</td>
                    <td className="px-3 py-2 font-mono text-gray-500 text-right">{row.hosts}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border border-amber-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2">
          <span className="text-lg">🔍</span>
          <span className="font-semibold text-amber-800">Mission Brief</span>
        </div>
        {open ? <ChevronUp className="w-4 h-4 text-amber-500" /> : <ChevronDown className="w-4 h-4 text-amber-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-amber-900">{LAB06_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-amber-700 uppercase tracking-wider mb-1.5">Issues reported:</p>
            <ul className="space-y-1">
              {LAB06_SCENARIO.reports.map(r => (
                <li key={r} className="text-sm text-amber-800 flex items-start gap-2">
          ...
          <div className="bg-white/70 rounded-xl p-3 border border-amber-100">
            <p className="text-sm text-amber-900 font-medium">{LAB06_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-amber-600">Lab 06</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1">
              <Star className="w-3 h-3 text-yellow-400" />{LAB06_META.difficulty}
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB06_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB06_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-amber-500" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">
            {LAB06_META.skillsTested.map(s => (
              <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5">
                <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" />{s}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-amber-500 to-orange-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
          ...
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed
                ...
                : sel ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
        ...
        <div className="mt-2 space-y-1">
          ...
          {question.correctOrder.map((item, i) => (
            <div key={item} className="flex items-center gap-2 text-xs text-gray-600">
              <span className="font-bold text-amber-600">{i + 1}.</span>{item}
            </div>
          ))}
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-6 text-center">
          ...
          <p className="text-amber-100 text-sm mt-1">{LAB06_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
            <div className="flex items-center gap-2 mb-2"><Trophy className="w-4 h-4 text-amber-600" /><span className="text-sm font-semibold text-amber-700">Concept Mastered</span></div>
            <p className="text-sm font-bold text-amber-800">{LAB06_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB06_COMPLETION.masteredPoints.map(p => (
              <li key={p} className="text-xs text-amber-700 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0" />{p}</li>
            ))}</ul>
          </div>
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB06_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB06_COMPLETION.nextLab.title}</p>
          ...
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab06Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab06', score, total)
  ...
  const isCalcQ   = currentQ.type === 'calculation'
  const isSubnetQ = currentQ.type === 'subnet-identification'
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => (
                <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-amber-400' : i === currentIdx ? 'bg-amber-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 section-enter">
              <div className="flex items-start justify-between gap-3 mb-5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isCalcQ   && <span className="text-xs text-orange-600 flex items-center gap-1"><Hash className="w-3 h-3" />Block size = 256 − last octet of mask</span>}
                  {isSubnetQ && <span className="text-xs text-amber-600 flex items-center gap-1"><Table2 className="w-3 h-3" />Use subnet blocks table</span>}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-amber-600 hover:bg-amber-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
                  Check Answer
                </button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              ...
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => (
                  <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-amber-400' : i === currentIdx ? 'bg-amber-400 opacity-60' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB06_META.lessonsReinforced.map(l => (
