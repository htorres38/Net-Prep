...
import {
  ...
  LAB16_META, LAB16_SCENARIO, LAB16_COMPLETION,
...
function TopologyCard({ highlightNodes }: { highlightNodes?: string[] }) {
  ...
  const Node = ({ name, icon, addr }: { name: string; icon: string; addr?: string }) => (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl border-2 transition-all duration-300 min-w-[110px]
      ${hi.has(name) ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-300 scale-105 shadow-md' : 'border-rose-300 bg-rose-50'}`}>
      <span className="text-lg">{icon}</span>
      <span className="text-xs font-bold text-gray-800">{name}</span>
      {addr && <span className="text-xs font-mono text-rose-700">{addr}</span>}
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">ACL Lab</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <div className="flex gap-4 justify-center">
          <Node name="PC-A" icon="💻" addr="192.168.1.50" />
          <Node name="PC-B" icon="💻" addr="192.168.1.10" />
        </div>
        <div className="w-px h-3 bg-rose-300" />
        <Node name="Router R1" icon="🌐" addr="G0/0: .1.1 | G0/1: .2.1" />
        <div className="w-px h-3 bg-rose-300" />
        <Node name="Server" icon="🖥️" addr="192.168.2.10" />
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100 space-y-1">
        <div className="flex justify-between text-xs">
          <span className="text-rose-600 font-semibold">G0/0</span>
          <span className="text-gray-400">192.168.1.0/24 (LAN)</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-rose-600 font-semibold">G0/1</span>
          <span className="text-gray-400">192.168.2.0/24 (Server)</span>
        </div>
      </div>
    </div>
  )
...
// acl ref
function ACLReferencePanel() {
  const [open, setOpen] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <span className="text-sm font-semibold text-gray-800">ACL Quick Reference</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">ACL Types</p>
            {[
              { type: 'Standard',  nums: '1–99',   color: 'bg-blue-50 text-blue-700',   desc: 'Source IP only — place near destination' },
              { type: 'Extended',  nums: '100–199', color: 'bg-rose-50 text-rose-700',   desc: 'Src+Dst+Proto+Port — place near source' },
            ].map(r => (
              <div key={r.type} className={`rounded-lg px-3 py-1.5 mb-1 ${r.color} border border-current border-opacity-20`}>
                <span className="text-xs font-bold">{r.type} ({r.nums})</span>
                <span className="text-xs opacity-80 ml-2">— {r.desc}</span>
              </div>
            ))}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Key Rules</p>
            <div className="bg-rose-50 border border-rose-200 rounded-lg p-2 space-y-1 text-xs text-rose-800">
              <p><span className="font-bold">Top-down</span> — first match wins, then stop</p>
              <p><span className="font-bold">Implicit deny</span> — blocks all unmatched traffic</p>
              <p><span className="font-bold">ip access-group</span> [ACL] in|out → apply to interface</p>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-1.5">Wildcard Masks</p>
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-2 space-y-1 text-xs text-gray-700 font-mono">
              <p>0.0.0.0 = match exact host</p>
              <p>0.0.0.255 = match /24 subnet</p>
              <p>255.255.255.255 = match any</p>
            </div>
          </div>
        </div>
      )}
...
function ScenarioCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-rose-50 to-red-50 rounded-2xl border border-rose-100 shadow-card overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-2"><span className="text-lg">🔍</span><span className="font-semibold text-rose-800">Mission Brief</span></div>
        {open ? <ChevronUp className="w-4 h-4 text-rose-500" /> : <ChevronDown className="w-4 h-4 text-rose-500" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <p className="text-sm text-rose-900">{LAB16_SCENARIO.context}</p>
          <div>
            <p className="text-xs font-semibold text-rose-700 uppercase tracking-wider mb-1.5">Reports:</p>
            <ul className="space-y-1">{LAB16_SCENARIO.reports.map(r => (
              <li key={r} className="text-sm text-rose-800 flex items-start gap-2"><span className="text-red-400 mt-0.5">!</span>{r}</li>
            ))}</ul>
          </div>
          <div className="bg-white/70 rounded-xl p-3 border border-rose-100">
            <p className="text-sm text-rose-900 font-medium">{LAB16_SCENARIO.challenge}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-rose-600">Lab 16</span>
            <span className="text-gray-300">·</span>
            <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Star className="w-3 h-3 text-yellow-400" />{LAB16_META.difficulty}</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{LAB16_META.title}</h1>
          <div className="flex flex-wrap gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{LAB16_META.estimatedTime}</span>
            <span className="flex items-center gap-1.5 text-sm text-gray-600"><BookOpen className="w-4 h-4 text-rose-400" />{QUESTIONS.length} Questions</span>
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0 max-w-xs">
          ...
          <ul className="space-y-1">{LAB16_META.skillsTested.map(s => (
            <li key={s} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-rose-400 shrink-0" />{s}</li>
          ))}</ul>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-rose-500 to-red-400 rounded-full transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2.5">
      {question.options.map(opt => {
        ...
        let cls = 'border-gray-200 bg-white hover:border-rose-300 hover:bg-rose-50 cursor-pointer'
        ...
        } else if (sel) cls = 'border-rose-400 bg-rose-50 ring-2 ring-rose-200 cursor-pointer'
        return (
          <button key={opt.key} onClick={() => !state.submitted && !state.revealed && onSelect(opt.key)} disabled={state.submitted || state.revealed}
            ...
            <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
              state.submitted || state.revealed ? correct ? 'bg-green-500 text-white' : sel ? 'bg-red-400 text-white' : 'bg-gray-200 text-gray-500' : sel ? 'bg-rose-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{opt.key}</span>
...
function SeqOptions({ question, state, onMove }: { question: SeqQuestion; state: QState; onMove: (f: number, t: number) => void }) {
  const COLORS = ['bg-rose-100 text-rose-700 border-rose-200', 'bg-red-100 text-red-700 border-red-200', 'bg-pink-100 text-pink-700 border-pink-200', 'bg-orange-100 text-orange-700 border-orange-200']
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden celebrate-bounce">
        <div className="bg-gradient-to-br from-rose-500 to-red-600 p-6 text-center">
          ...
          <p className="text-rose-100 text-sm mt-1">{LAB16_META.title}</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-rose-50 rounded-2xl p-4 text-center border border-rose-100">
              <p className="text-3xl font-bold text-rose-600">{score}/{total}</p>
              <p className="text-xs text-rose-500 font-medium mt-0.5">Correct</p>
            </div>
          ...
          <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
            ...
            <p className="text-sm font-bold text-green-800">{LAB16_COMPLETION.conceptMastered}</p>
            <ul className="mt-2 space-y-1">{LAB16_COMPLETION.masteredPoints.map(p => (
          ...
          {pct < 80 && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <ul className="space-y-1">{LAB16_COMPLETION.reviewIfNeeded.map(l => (
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
            ...
            <p className="text-sm font-semibold text-gray-800">{LAB16_COMPLETION.nextLab.title}</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/labs" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-semibold text-sm transition-all">Back to Labs</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function Lab16Page() {
  ...
  const handleContinue = useCallback(() => {
    if (currentIdx + 1 >= total) {
      setLabCompletion('lab16', score, total)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {QUESTIONS.map((q, i) => <div key={q.id} className={`w-2 h-2 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-rose-500 scale-125' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-200'}`} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!qState.submitted && !qState.revealed && (
                <button onClick={checkAnswer} disabled={!canCheck} className={`ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all ${canCheck ? 'bg-rose-600 hover:bg-rose-700 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Check Answer</button>
              )}
              ...
              {qState.submitted && (qState.isCorrect === true || qState.revealed) && (
                <button onClick={handleContinue} className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-semibold text-sm shadow-md transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-rose-600">{score} correct</span>
              </div>
              <div className="flex gap-1">
                {QUESTIONS.map((q, i) => <div key={q.id} title={`Q${i + 1}`} className={`flex-1 h-2.5 rounded-full transition-all ${correctSet.has(q.id) ? 'bg-green-400' : i === currentIdx ? 'bg-rose-400' : i < currentIdx ? 'bg-gray-300' : 'bg-gray-100'}`} />)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {LAB16_META.lessonsReinforced.map(l => (
